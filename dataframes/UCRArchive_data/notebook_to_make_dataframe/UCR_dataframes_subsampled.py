...
Dataset = 'HandOutlines'  # "ECG5000" "ECG200"  "ECGFiveDays" "TwoLeadECG" "Beef" "GunPoint" "GunPointMaleVersusFemale"
subsampled = 5
save_path = f'/Users/vickyhaney/Documents/GAship/DrBruno/EKG/UCRArchive_data/{Dataset}/subsampled_{subsampled}'
...
# subsample everyother point for each test signal (1024 - > 512)
test_subsampled = test.iloc[:, 1:].values[:, ::2]
print(f"Subsampled test data shape: {test_subsampled.shape}")
test_subsampled = pd.DataFrame(test_subsampled)
print(test_subsampled.head())

plt.figure(figsize=(10, 5));
x_subsampled = np.linspace(0, 1025, (length)//2)
x_true = np.linspace(0, 1024, length-1)
plt.plot(test_subsampled.iloc[0, :])
#plt.plot(x_true, test.iloc[0, 1:], alpha=0.5)
plt.title(f"Subsampled test label: {test.iloc[0, 0]}")
plt.show()
print(test_subsampled.shape)
print(test_subsampled.transpose().head)
...
for label in test[0].unique():
    ...
    df = df.transpose()
    # subsample every other row
    df = df.iloc[::subsampled, :] #
    print(df.shape)
...
list_class_1 = list(train[train[0] == 1].index)
print(list_class_1)
print(len(list_class_1))

list_class_2 = list(train[train[0] == 2].index)
print(list_class_2)
...
# subsample every other point for each train signal (1024 - > 512)
...
# subsample every other point for each train signal (1024 - > 512) then relabel the index
globals()[f"{Dataset}_train_transpose"] = globals()[f"{Dataset}_train_transpose"].iloc[::subsampled, :]

globals()[f"{Dataset}_train_transpose"].reset_index(drop=True, inplace=True)
