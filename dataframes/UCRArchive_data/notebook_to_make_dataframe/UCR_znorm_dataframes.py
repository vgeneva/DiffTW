...
Dataset = 'SemgHandSubjectChtwo'  # "ECG5000" "ECG200"  "ECGFiveDays" "TwoLeadECG" "Beef" "GunPoint" "GunPointMaleVersusFemale"
...
# Z-normalize the data (excluding the label column)
test_features = test.iloc[:, 1:]
test_normalized = (test_features - test_features.mean(axis=1).values[:, None]) / test_features.std(axis=1).values[:, None]

# Combine the labels with the normalized features
test = pd.concat([test.iloc[:, 0], test_normalized], axis=1)
print(test.head())
print(test.tail())
...
train_normalized = (train_features - train_features.mean(axis=1).values[:, None]) / train_features.std(axis=1).values[:, None] 

train = pd.concat([train.iloc[:, 0], train_normalized], axis=1)
